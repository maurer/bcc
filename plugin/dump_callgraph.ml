...
let dump (proj : Project.t) (out_name : string) : unit =
 ...
 let node_attrs tid =
    ...
    [`Label (Memmap.lookup mem addr |> Sequence.find ~f:(fun (_, v) -> Value.is Image.symbol v) |> Option.map ~f:(fun (_,v) -> Value.get_exn Image.symbol v) |> Option.value ~default:node_name)] in
  Graphlib.to_dot (module Graphlib.Callgraph) ~filename:out_name ~node_attrs cg
